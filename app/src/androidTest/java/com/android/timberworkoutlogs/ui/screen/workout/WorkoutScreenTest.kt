...
class WorkoutScreenTest : TestCase() {
    ...
    // Without this the exercise catalog is only present if some earlier test class happened to
    // seed it, so running this class on its own found no "Barbell Bench Press" to select.
    @get:Rule(order = 1)
